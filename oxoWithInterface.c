...
validity valid(game *g, char *text) {
    ...
    if (row2 != 'a' && row2 != 'b' && row2 != 'c') return BadLetter;
    else if (col2 != '1' && col2 != '2' && col2 != '3') return BadDigit;
    else if (g->grid[r][c] != 2) return BadCell;
    else return OK;
...
player line(player p0, player p1, player p2) {
    ...
    else if (p0 == p1 && p1 == p2 && p0 == O) return O;
    else return N;
}
...
player win(game *g) {
    int winner = N;
    if (winner == N) winner = line(g->grid[0][0], g->grid[0][1], g->grid[0][2]);
...
bool draw(game *g) {
    if (win(g) == X || win(g) == O) return false;
    else if (g->moves == 9) return true;
    else return false;
}
...
void play(char player) {
  ...
  else if (win(&game) == X )printf("X Wins\n");
  else printf("O Wins\n");
}
