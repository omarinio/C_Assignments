...
int convert(int n, const char mark[]) {
    ...
    switch (n) {
      case 1:
        if (!isdigit(mark[0])) mark2 = -1;
        else mark2 = atoi(mark);
        break;

      case 2:
        if (!isdigit(mark[0])) mark2 = -1;
        else if (!isdigit(mark[1])) mark2 = -1;
        else if (mark[0] == '0') mark2 = -1;
        else mark2 = atoi(mark);
        break;
      ...
      case 3:
        ...
        else if (!isdigit(mark[2])) mark2 = -1;
        else if (mark[0] == '0') mark2 = -1;
        else mark2 = atoi(mark);
        break;
    }

    if (mark2 > 100) mark2 = -1;
...
int grade(int m) {
    int grade;
    if (m >= 70 && m <= 100) grade = G1;
    else if (m >= 60 && m <= 69) grade = G21;
    else if (m >= 50 && m <= 59) grade = G22;
    else if (m >= 40 && m <= 49) grade = G3;
    else if (m >= 0 && m <= 39) grade = Fail;
    else grade = Invalid;
    return grade;
}
