...
list *newList(item d) {
    ...
    *newlist->current = (node) {newlist->pre, newlist->pre,0};
    *newlist->pre = (node) {newlist->current, newlist->current, -1};
...
item getF(list *l) {
    ...
    else return l->current->data;
}

item getB(list *l) {
    if (endB(l)==true) return l->pre->data;
    else return l->current->prev->data;
}
...
bool deleteF(list *l) {
    if((l->current==l->pre)|| (l->current->next == l->pre) ) return false;
    else{
      node *prevNode = l->current->prev;
      l->current = l->current->next;
      free (l->current->prev);
...
bool deleteB(list *l) {
    if (l->current->prev == l->pre) return false;
    else{
